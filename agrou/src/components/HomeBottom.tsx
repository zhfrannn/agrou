...
import { 
  Building2, Shovel, ShoppingBag, ShieldCheck, 
  Store, ArrowRight, Star, 
  TrendingUp, Handshake, CheckCircle2,
  UserCheck, Coins, MessageSquare, PackageCheck, Search, BadgeCheck, ShoppingCart, Calendar
} from "lucide-react";
...
const STEPS_DATA: Record<string, Array<{
  icon: any;
  title: string;
  desc: string;
  color: string;
...
const TESTIMONIALS = [
  {
    ...
    quote: "Dulu anggota jalan sendiri-sendiri. Setelah pakai Agrou, kami bisa beli obat bareng (Tani) plus jual kopi (Pasar) ke luar pulau. Omzet desa naik 40%!",
  ...
  },
  {
    ...
    quote: "Sistem pembagian hasilnya luar biasa. Tidak ada lagi cekcok antar nelayan, karena semua terekap otomatis saat ikan asin kami laku di Pasar.",
  ...
  },
  {
    ...
    quote: "Berkat riwayat belanja proteksi di Tani, produk padi kami langsung dapat lencana Verified. Pembeli dari kota lebih percaya.",
    ...
    <div className="w-full bg-(--color-cream)">
      ...
      <section className="pt-12 pb-16 max-w-360 mx-auto px-8">
        <div className="text-center mb-16">
          <h2 className="font-display font-black text-4xl lg:text-5xl text-gray-900 mb-4">Cara Kerja Agrou</h2>
              ...
              {STEPS_DATA[activeTab].map((step, idx) => {
                ...
                return (
                  <div key={idx} className="flex flex-col items-center text-center relative group">
                    ...
                    <p className="text-gray-600 font-medium leading-relaxed px-4 text-sm">{step.desc}</p>
                  </div>
                );
              })}
            </motion.div>
      ...
      <section className="pt-16 pb-16 relative overflow-hidden">
        ...
        <div className="max-w-360 mx-auto px-8 relative z-10">
          <div className="text-center mb-16">
             <h2 className="font-display font-black text-4xl text-gray-900">Koperasi yang Sudah Merasakan Manfaatnya</h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {TESTIMONIALS.map((t, idx) => (
              ...
              >
                <div className="absolute top-6 right-8 text-(--color-lime) opacity-30 text-6xl font-serif">"</div>
                ...
                <div className="flex items-center gap-4 pt-6 border-t border-gray-100">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-gray-500 mb-0.5">{t.role}</p>
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-bold text-(--color-orange)">{t.location}</span>
                      <span className="bg-(--color-lime) text-(--color-forest-dark) border border-(--color-lime)/20 text-[10px] font-bold px-1.5 py-0.5 rounded flex items-center gap-1">
                         <CheckCircle2 size={10} className="fill-(--color-forest-dark)/10 text-(--color-forest-dark)" /> Verified
                      </span>
                    </div>
                  </div>
                </div>
          ...
          <div className="bg-linear-to-r from-(--color-forest) to-(--color-orange) rounded-[3rem] px-8 py-16 md:p-20 relative overflow-hidden shadow-2xl flex flex-col md:flex-row items-center justify-between gap-12">
            ...
            <div className="max-w-xl relative z-10 text-center md:text-left">
              <h2 className="font-display font-black text-4xl lg:text-5xl text-white mb-6 leading-tight">
                Siap bergabung dengan <span className="text-(--color-lime)">2.400+</span> koperasi aktif?
              </h2>
              <p className="text-white/90 text-lg font-medium mb-0">
                Modernisasi desa Anda hari ini. Lindungi lahannya, jual produknya, dan kelola anggotanya dengan mudah.
              </p>
            </div>
